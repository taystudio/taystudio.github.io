...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-10 (**모니터 PPI·시야거리 도구 + 쿠팡 파트너스 위젯 8개 라이브 + 사이트 일관성 38선/60도구 + 쿠팡 affiliate AdSense 승인 전까지 CSS hide + 헤더 깜박임 완전 해결 + 영어판 28선 일괄 launch ✅** — 5/9~5/10 8 commit 체인. tools/monitor/ 신규 (PPI·픽셀 피치·시야거리·4K 변별 + 게이밍·사무·영상편집·세컨드 4 추천 카드, JSON-LD WebApp+Breadcrumb+FAQ 7건). 쿠팡 파트너스 가입(회원 ID AF4086854) + Playwright+CDP로 너 Chrome retail 쿠팡 직접 스크래핑(Akamai 봇 차단 우회) → 카테고리별 베스트셀러 1픽업 → 콘솔에서 위젯·추천링크 8개 발급 → 4 카드에 박음. AdSense 리뷰어가 affiliate 비중 오인할 위험 차단 위해 CSS hide 처리(승인 후 단일 블록 삭제로 즉시 unhide). 헤더 깜박임 = site-chrome.js를 70 HTML 파일 모두 `</body>`→`</head>` blocking 이동 + min-height 예약 + install 버튼 eligibility localStorage 캐시 → 매 페이지 navigation 시 헤더 컨텐츠 유지. **영어판 일괄 launch (5/10)** = Phase A baseline + Phase B 28선 5 agent 병렬(image 9·pdf 5·video 5·text 1·calc 7) + SEO audit 9 영역 통과 + home Phase B 반영 fix + Gap 2 manifest 다국어. 한 세션 22분 wall time. 한국 source 31파일 hreflang 양방향 + sitemap 71→102 URL + site-chrome.js i18n + plan_en.md 운영 doc 분리 ([`plan_en.md`](./plan_en.md)). commit `a6a60a8`(launch 104 file)·`ea9e3bc`(Gap 2 + Image fix). **앞선 작업 — IndexNow 인프라(2026-05-07) + 저장공간 계산기(2026-05-06, 디지털·기기 카테고리 신설)** + AI 이미지 업스케일링 봉인 + privacy·terms root 이전. **현 시점 도구 수 한국 60선 + 영어 28선**, tools 카테고리 8개(디지털·기기 = 저장공간·모니터 2선), **sitemap 104 URL**(`/en/privacy/`·`/en/terms/` 추가). **+ 영어판 privacy/terms 신규 launch (5/10 후속) + plan_integration.md 신규 (한·영 양방향 그라운드 룰 + monetization 양방향 룰 §9 — AdSense 공통/쿠팡 한국 전용/영어 affiliate 미래)** — 영어 footer 404 해결 + KO privacy/terms hreflang 양방향 alternate 추가. **다음 = AdSense 신청·승인 대기(쿠팡 affiliate hide 풀기 트리거) + 영어판 cross-category nav 보강·번역 검수** (§10·plan_en.md §9.8 참고). 다음 세션이 이 문서 보고 이어서 작업.
...
10. **🌐 영어 시장 진입 — `/en/` path 일괄 launch ✅ (2026-05-07 초안 → 2026-05-10 Phase A·B·SEO·home·Gap 2 일괄)** — 글로벌 트래픽·AdSense 영어 RPM($3~8, 한국 대비 ~3배) 노림. **권장 시점 조건(한국 5000+ MAU·AdSense 안정 수익) 미충족 상태에서 사용자 결정으로 조기 진입**. **5/10 한 세션에 28선 전부 LIVE** — 5 agent 병렬 dispatch로 sequential 8시간 작업을 22분으로 단축. **운영 doc 분리** — `history/plan/plan_en.md` 신설. 상세 = [`history/plan/plan_en.md`](./plan_en.md)
    - **✅ Phase A — Pilot baseline (2026-05-10)**: `compound`(복리 계산기) 영어판 1선 + `/en/` 디렉토리 구조(`en/index.html`·`en/tools/index.html`·`en/tools/compound/`) + 한국판 디자인 1:1 mirror(hub-bg·hub-grid 5카드·quick-list). 5/7 stash(`english-pilot-wip`) 복원 + 4 conflict resolve + `taystudios.com` 도메인 일괄 sed (4파일 × ~40 stale URL). descender clearance fix(`.hub-title { padding-bottom: 0.18em }`). plan_en.md §9.3
    - **✅ Phase B — universal 28선 일괄 (2026-05-10)**: 5 agent 병렬 (image 9 / pdf 5 / video 5 / text 1 / calc 7) — 32 file group, ~22분 wall time. **부수 작업 동시 처리**: site-chrome.js `TRANSLATED_PATHS` 33 path · sitemap.xml 71→102 URL · 한국 source 31파일 hreflang 양방향 · 4 카테고리 hub 신설(`/en/image/`·`/en/pdf/`·`/en/video/`·`/en/text/`). 한국 특화 32선(salary·취득세 등 법령·세율 종속)은 ko-only 유지 — 번역 X. plan_en.md §9.4
    - **✅ SEO audit + HowTo parity (2026-05-10)**: 9 영역 audit — 34/34 canonical·hreflang·OG·BreadcrumbList·AdSense·GA4 production-ready. Gap 1(`image/id-photo`·`image/qr-gen` 영어판 HowTo JSON-LD 누락 2건) fix → 한·영 HowTo schema 9/9 parity 회복. plan_en.md §9.5
    - **✅ Home Phase B 반영 fix (2026-05-10)**: Phase B 마무리에서 `/en/` home 갱신 누락(hub-grid 4 카드 `coming` + quick-list "coming soon") — 사용자 지적 후 수정. 5 카드 LIVE 활성·quick-list 19 link·SEO meta·JSON-LD `hasPart` 5 entry. commit `a6a60a8` (영어판 첫 launch — 104 file). plan_en.md §9.6
    - **✅ Gap 2 manifest 다국어 (2026-05-10)**: `/en/manifest.webmanifest` 신규(scope=/en/, lang=en-US, shortcuts 4 영어 path = compound·BMI·image-compress·pdf-merge) + 34 `/en/*.html` link 갱신. 영어 PWA 설치 시 영어 메타·영어 단축키 노출, scope 격리로 한·영 PWA 동작 분리. 사이드 fix(같은 commit) = `en/index.html` quick-list Image row "Background remove" → "Remove BG" (라벨 wrap 해소). commit `ea9e3bc`. plan_en.md §9.7
    ...
    - **✅ TODO-A: 커스텀 도메인 — DONE (2026-05-09 `taystudios.com` migration 완료)** — 영어 출시 전 선제 migration. 상세 = §9.5. 결과: en/ 시드 URL은 처음부터 `taystudios.com/en/*`로 SEO 누적
    - **✅ TODO-B: Phase B universal 28선 — DONE (2026-05-10)**: 위 §9.4 일괄 launch
    - **TODO-C: 영어 SEO·트래픽 채널** (한국 채널과 완전히 별개) — Pilot 검증 1~2주 후 진입:
      ...
      - **Search Console international targeting** — 영어판 출시 후 설정 (GSC `taystudios.com` URL prefix property 이미 검증됨)
      - **장기**: 추후 대형 커뮤니티 확장 — 사용자 결정 사항
    - **✅ TODO-D: i18n 미세 보강** — Phase B 진입 시 대부분 처리 + 5/10 audit 후속 4 step 일괄:
      - ✅ `/en/manifest.webmanifest` 다국어 — 5/10 Gap 2 처리
      - ✅ `<html lang>`·`<og:locale>` 자동 감지 — site-chrome.js i18n으로 처리
      - ✅ sitemap.xml `/en/*` URL — 71→102 (32 URL 추가)
      - ✅ site-chrome.js 언어 토글 — `getAltLangUrl()` + `TRANSLATED_PATHS` whitelist 33 path
      - ✅ og-image 영어 변형 — `og-image-en.png` 신규 (1200×630, "Useful things." + privacy 차별화 + 5 카테고리 pill) + 영어 34 file의 OG·Twitter·Organization image 갱신 (5/10 plan_en §9.8 step 3)
      - ✅ cross-category nav 보강 — `en/image/`·`en/pdf/`·`en/video/`·`en/text/`·`en/tools/` 5 hub 모두 자기 제외 4 cross-link (5/10 plan_en §9.8 step 1)
      - ✅ 한국 manifest "TayTools" → "TAYSTUDIO" 브랜드 통일 (5/10 plan_en §9.8 step 2)
      - ✅ 5 카테고리 번역 검수 doc 풀세트 — `history/seo/2026-05-10-en-{image,pdf,video,text,calculators}-translation-review.md` 87KB 총 (image 26KB · pdf 15KB · video 15KB · text 5KB · calc 26KB) (5/10 plan_en §9.8 step 4)
      - ⚠ AI 초안 번역 품질 사용자 검수 — 5 카테고리 doc 풀세트 준비됨, 사용자 review 대기 (다음 1순위, image → pdf → video → calc → text 순 권장)
      - ✅ Smart lang banner + 헤더 토글 강조 — site-chrome.js `BROWSER_LANG` mismatch 감지 + dismiss·CTA·lang-pref localStorage 기억 + 양방향(root↔/en/). 헤더 lang-toggle 글로브 emoji + box style 강조. SEO 무영향(auto redirect 안 함, 정적 HTML banner markup 0건). 한국 사용자 영향 0(브라우저 mismatch 시에만 노출). (5/10 plan_en §9.9)
      - ✅ Hero 환영 카드 — root·/en/ home hero 안에 큰 카드 박힘(영어/한국어 환영 텍스트 + 큰 CTA 버튼 + ×). banner와 같은 mismatch 조건 + localStorage 키 공유로 dismiss 일관. 외국인 root 진입 시 "한 번에 발견" UX, banner는 보조. 한국 사용자 영향 0. (5/10 plan_en §9.10)
      - ✅ 외국인 사용자 구별법 concept doc — `history/seo/2026-05-10-foreign-user-detection.md` 신설. `navigator.languages` 메커니즘 + OS·브라우저 결정 chain + 한계(영/한 only·false positive·VPN·SW 캐시) + 5 시뮬·자가 검증 방법(DevTools Sensors·시크릿 창·Console·SW 격리·playwright) + 자가 진단 체크리스트. (5/10 plan_en §9.10)
      - ✅ **영어 privacy/terms 신규 launch (5/10 후속)** — Phase B 일괄 launch 시 누락분. site-chrome.js footer가 `/en/privacy/`·`/en/terms/`로 link 거는데 target 부재로 production 404. 한국 source mirror 후 영어 적응(GDPR·CCPA·COPPA 명시·governing law mandatory law 단서). 시행일 = 2026-05-10(영어 launch 시점, 한국 시행일과 독립). 4 페이지 hreflang 양방향 정합(KO privacy/terms도 alternate 추가) + sitemap.xml `/en/privacy/`·`/en/terms/` 2 URL 추가(102→104) + canonical·og-image-en·manifest scope `/en/`·footer 5 hub link 모두 200 검증. 본 작업이 plan_integration.md §3 분류 = 공통(universal) 정책 페이지 적용 1차 사례
      - ✅ **plan_integration.md 신규 — 한·영 양방향 그라운드 룰 (5/10)** — `history/plan/plan_integration.md` 신설(331줄 / 11KB). plan.md·plan_en.md 분리 후 양 시장 운영의 그라운드 룰 누락. §0 핵심 원칙(양쪽 default·분류 우선·컨센서스 일치) / §1 결정 트리(KO 종속? · 영어권 1:1 가능? · EN 종속?) / §2 카테고리 현황 / §3 신규 추가 체크리스트(분류별) / §4 기존 수정 동기화 / §5 양방향 검수 bash / §6 anti-pattern / §7 Q&A / §8 세션 진입 절차 / **§9 monetization 양방향 룰**(AdSense 공통 / 쿠팡 한국 전용 / Amazon Associates 미래 EN-only / GA4·GSC 공통 / 신규 채널 결정 트리 / monetization anti-pattern). 매 세션 진입 시 plan.md·plan_en.md와 함께 무조건 읽는 ground rule doc. 신규 도구·UI·monetization 작업의 분류 누락 + 한쪽만 작업하는 mismatch 방지가 목표
...
12. **📌 `plan.md` ↔ `plan_en.md` 동기화 추적** (2026-05-10 등재 / **5/10 후속 = `plan_integration.md` 신설로 운영 룰 명문화**) — 두 plan 분리 후 한국판 변경 시 영어판도 갱신해야 하는 비용 발생. **현 상태 = 영어판은 한국판 §0 컨텍스트만 요약 흡수, §1~§9는 영어 전용** → 동기화 영역은 §0(컨텍스트), §0.4(migration 상태), monetization 정책 정도로 좁음. 분기 심해질수록 자연스럽게 별도 트랙. **2026-05-10 후속 = `plan_integration.md` 신설**로 두 plan 위 그라운드 룰 doc 생김. 한·영 양방향 분류·체크리스트·monetization 룰을 단일 doc로 운영 → 동기화 부담 doc(plan.md·plan_en.md)에서 결정 doc(plan_integration.md)으로 이전. **모니터링 트리거**: 한국판 §0 변경 시 영어판 §0 갱신 여부 확인 + 분류·monetization 룰 변경 시 plan_integration.md §1·§9 갱신. 진행 후보:
    - 한국 plan.md §0(본질·인프라·monetization) 변경 시 = 같은 PR에서 plan_en.md §0 + plan_integration.md §9 갱신 여부 확인
    - 한국 §9.5 migration 완료 시(이미 5/9 완료) = 영어판 §0.4 일치 확인 (5/10 정합)
    - 영어판 사용자 결정·우선순위 변경 시 = 한국판 §9.4-10에 한 줄 미러 (양방향 시너지)
    - 분류·monetization 룰 변경 = `plan_integration.md` 단일 source of truth
    - **분기 임계점**: 영어판이 자체 §1~§9 발전해서 plan_en.md가 500줄 초과하면 한·영 별도 운영 일관 인정. 동기화 영역은 §0뿐 + plan_integration.md 룰
