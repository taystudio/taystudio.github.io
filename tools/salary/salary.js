...
const RATES = {
  ...
  국민연금_월상한: 6370000,    // 2025.7~2026.6 적용 (637만원) (2026.7.1부터 6,590,000원으로 인상 예정 — 시행 시 갱신)
  건강보험: 0.03595,          // 2026년 시행 (직장가입자 기준)
  장기요양료율: 0.009448,      // 2026년 보수월액 직접 비율 (NHIS 산정 공식)
  고용보험: 0.009,            // 고용보험법 제12조
};
...
function 세액공제한도(연봉) {
  if (연봉 <= 33000000) return 740000;
  if (연봉 <= 70000000) return Math.max(660000, 740000 - (연봉 - 33000000) * 0.008);
  if (연봉 <= 120000000) return Math.max(500000, 660000 - (연봉 - 70000000) * 0.5);
  return Math.max(200000, 500000 - (연봉 - 120000000) * 0.5);  // 제59조 4구간: 1.2억 초과 50만→20만 phase-down
}
...
function 계산(연봉, 부양가족수) {
  ...
  const 산출 = 산출세액(과세표준);
  const 세액공제_raw = 근로소득세액공제(산출);
  const 세액공제 = Math.min(산출, 세액공제_raw, 세액공제한도(연봉));
