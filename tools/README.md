...
한국에서 자주 쓰는 30가지 계산기를 정적 사이트로 묶은 모음. AdSense + 쿠팡파트너스 수익화 목적, 출처·법령 근거 명시 정책.

---

## 1. 한눈에 보기

- **30개 계산기** 전부 클라이언트사이드 JavaScript (백엔드 없음)
- **GitHub Pages** 배포 가정 (`https://taehyuklee.github.io/Archive/tools/`)
- 각 도구는 디렉토리 1개 = `index.html` + `<name>.js` 구조
- **공통 CSS**: `tools/css/style.css` (다크모드·반응형 자동 지원)
- **사이트맵**: `tools/sitemap.xml`
...
| 도구 | 출처 / 갱신 주기 |
|---|---|
| 중개수수료 | 공인중개사법 시행규칙 §20 별표 (2024.10 개정) |
| 재산세 | 지방세법 §111·§111의2 / 매년 7·9월 분납 |
| 종합부동산세 | 종부세법 §8·§9 / 매년 12월 정기고지 |
| 양도소득세 | 소득세법 §89·§95·§103·§104 / 정책 변경 잦음 |

### 금융·세금

| 도구 | 출처 / 갱신 주기 |
|---|---|
| 연봉 실수령액 | 4대보험법 + 소득세법 §47·§55·§59 / 매년 갱신 |
| 연말정산 | 소득세법 §47~§59, 조특법 §126의2 / 매년 1~2월 |
| 종합소득세 | 소득세법 §55 / 매년 5월 신고 |
| 부가가치세 | 부가세법 §30 (10% 고정) |
| 상속세 | 상증법 §18·§19·§26·§69 / 개정 모니터링 |
| 4대보험료 | 4대보험법 (각각) / 매년 변경 |
| 대출 이자 | 수학 공식 (PMT) / 변경 없음 |
| 적금 이자 | 소득세법 §129 (이자세 15.4%) |
| 복리 | 수학 공식 / 변경 없음 |
...
| 주휴수당 | 근로기준법 §55 |
| 연차수당 | 근로기준법 §60 |
| 퇴직금 | 근로기준법 §2, 퇴직급여보장법 §8 |
| 실업급여 | 고용보험법 §45·§46 / 매년 상·하한 변경 |
| 육아휴직 급여 | 고용보험법 §70 / 2025년 개정 (연 1회 변경 가능) |
...
| 도구 | 출처 / 갱신 주기 |
|---|---|
| 자동차 취득세 | 지방세법 §12, 조특법 §66의4 (감면 일몰) |
| 연간 자동차세 | 지방세법 §127 / 매년 6·12월 분납 |
| BMI | 대한비만학회 2018 기준 |
| 칼로리(BMR/TDEE) | Mifflin-St Jeor 공식 (1990) |
| 만 나이 | 행정기본법 §7의2 (2023.6.28 시행) |
| D-day | 단순 날짜 계산 |
