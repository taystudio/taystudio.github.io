...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-06 (**저장공간 계산기 출시 — `tools/storage/` + 디지털·기기 카테고리 신설**. 쿠팡 파트너스 친화 도구 톱 1순위 채택. 영상·사진 용량 추정 + 외장 SSD/HDD/NAS 권장 카테고리. 추천 카드는 텍스트 권장만(쿠팡 카드는 승인 후 별도 작업). 도구 수 58 → **59선**, 계산기 36 → **37선**, sitemap 66 → **67 URL**, tools 카테고리 7 → **8개**(디지털·기기 신설). **앞선 작업 — AI 이미지 업스케일링 봉인** + privacy·terms root 이전 + 이미지 Tier 1 신규 3종 + PDF 카테고리 신설. 카테고리 5개(계산기 37 + 텍스트 3 + 이미지 9 + PDF 5 + 동영상 5). 다음 세션이 이 문서 보고 이어서 작업.
