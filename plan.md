...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-05 (**PWA UX 마감 — 비활성 토글 + 카카오 in-app 회수 + 모바일 헤더 컴팩트** — install 버튼 = 자동 숨김 X + 설치 후 회색 비활성 "✓ 설치됨"(localStorage 영구). 카톡·라인·페북·인스타·네이버 in-app UA 감지 → 보라 "외부 열기" 버튼으로 자동 전환 + Android Chrome intent fallback 모달. 모바일 헤더 480px·360px 미디어 쿼리로 컴팩트(따닥따닥 fix). + install 버튼 + 아이콘 자산 분리 + TayTools + PWA 통합 A안 완료 + favicon.ico + rotate 미리보기 UX + video wrapper hang fix 부분 검증. 카테고리 = `tools / text / image / video` **4개**. sitemap **60 URL** · 도구 수 **53선**. 다음 세션이 이 문서 보고 이어서 작업.
