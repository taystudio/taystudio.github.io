...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-05 (image/ 7개 도구 모두 출시 — batch 2·3 일괄 + 라이브러리 4종 vendoring) / 다음 세션이 이 문서 보고 이어서 작업.
...
├── image/                   ← taystudio.github.io/image/     (2단계, ✅ 완료 — 7선 + vendor 4종)
│   └── vendor/                                                (qrcode·jsQR·pdf-lib·tesseract)
├── test/                    ← taystudio.github.io/test/      (3단계 — 미진입)
└── (이후 ROI 재평가 후) luck/, quiz/, random/, track/
```

**현 시점(2026-05-05) 사이트 도구 수 = 43선** (계산기 33 + 텍스트 3 + 이미지 7), sitemap 49 URL.
...
| 2️⃣ | **image** (이미지·PDF) | **매우 큼** | 일반인 ✓ | ✓ (lib 무거움) | 중간 | **✅ 완료 (2026-05-05)** — 7선 (compress·resize·qr-gen·qr-scan·pdf-merge·pdf-split·ocr) |
...
| **qr-gen** — QR 코드 생성 | ★★★ | ✓ | qrcode-generator (~20KB) | 2 | ✅ 출시 (2026-05-05) |
| **qr-scan** — QR 코드 스캐너 | ★★ | ✓ | jsQR (~251KB) | 2 | ✅ 출시 (2026-05-05) |
| **pdf-merge** — PDF 합치기 | ★★★ | ✓ | pdf-lib (~513KB) | 3 | ✅ 출시 (2026-05-05) |
| **pdf-split** — PDF 자르기 | ★★★ | ✓ | pdf-lib | 3 | ✅ 출시 (2026-05-05) |
| **ocr** — 이미지 OCR | ★★ | ✓ (WASM) | Tesseract.js (~65KB + 학습데이터 lazy) | 3 | ✅ 출시 (2026-05-05) |
...
| pdf-merge, pdf-split | `image/vendor/pdf-lib.min.js` | 513KB | MIT |
| ocr | `image/vendor/tesseract.min.js` | 65KB | Apache-2.0 |

OCR 학습 데이터·WASM core·worker JS는 **첫 사용 시 jsdelivr CDN에서 자동 다운로드 → IndexedDB 캐시** (한국어 ~13MB, 이후 offline 가능). repo 비대 방지로 vendoring 안 함 — Tesseract.js의 표준 패턴.

### 4.4 진행 기록

**Batch 1 (2026-05-05)**
- [x] `common/css/style.css` 신규 클래스 — `.privacy-box`, `.file-drop-zone`, `.image-preview`, `.image-meta`, `.image-actions`, `.range-row`
- [x] `image/index.html` 허브 (초기: 2 활성 + 5 disabled)
- [x] `image/compress/` — 화질 슬라이더 + JPEG/PNG/WebP + 용량 감소율
- [x] `image/resize/` — px·% 토글 + 비율 유지 + 포맷 변환

**Batch 2 + 3 (2026-05-05, 일괄)**
- [x] `image/vendor/` 신설 + 라이브러리 4종 vendoring (qrcode 20KB / jsQR 251KB / pdf-lib 513KB / tesseract 65KB)
- [x] `image/qr-gen/` — 텍스트·URL·Wi-Fi → PNG·SVG, 오류복구 L/M/Q/H 선택
- [x] `image/qr-scan/` — 이미지 + 카메라(getUserMedia) 실시간 스캔, URL/tel/Wi-Fi 자동 분류
- [x] `image/pdf-merge/` — 다중 PDF 누적·정렬(↑·↓·✕) → 1 파일 병합
- [x] `image/pdf-split/` — `1-3,5,7-10` 범위 파싱(역순·반복 지원) → 새 PDF
- [x] `image/ocr/` — Tesseract.js 한국어/영어/혼합 + progress UI + 페이지 내 정확도 disclaimer
- [x] `image/index.html` 허브 disabled 5장 → 활성, ItemList JSON-LD 7개 완성
- [x] 루트 `index.html` — image hub 카드 "7선", hasPart description 7선 명시, 미리보기 섹션 6 카드, meta·OG·title 갱신
- [x] `scripts/build-sitemap.sh` 재빌드 → 44 → **49 URL**
- [x] `history/index.html` timeline article 추가 (batch 2+3 완성)
- [x] 검증: HTML 태그 밸런스 / FAQ DOM 6 = JSON-LD 6 (5개 도구 모두) / JS 문법 / JSON-LD 파싱
...
- **image/ batch 분할**: 7개를 3-way batch (canvas 2 → QR 2 → PDF 2 + OCR 1)로 출시 (4.1) — **2026-05-05 batch 1 출시 후 사용자 요청으로 batch 2·3 일괄 처리, 7개 모두 LIVE**
- **라이브러리 vendoring**: `image/vendor/`에 4종 (qrcode·jsQR·pdf-lib·tesseract) 직접 commit. CDN 의존 없음. 단 OCR 학습 데이터·worker·WASM core는 jsdelivr CDN + IndexedDB 캐시 (Tesseract 표준)
- **OCR 페이지 내부 disclaimer 정책**: 정확도 변수 큰 도구는 사이트 banner 대신 페이지 내 인라인 박스(`.ocr-disclaimer`) 사용 (7.2)
...
- [ ] sitemap.xml 단일 vs 카테고리별 분리 (현재 단일 root, **49 URL** — test/ 추가 후 60+ 예상, 그 시점에 분리 검토)
...
1. **test/ 카테고리** (3단계, plan §5) — vision·color-blind·hearing·reaction·typing 5종. SNS 바이럴·재방문 도구. 각 도구 페이지 내부 disclaimer (image/ocr 패턴 재사용 — 의료 진단 대체 X)
2. **PWA 통합 (A안 — 루트 manifest)** — image/ 7개 출시로 진입 시점 도달. 작업: ① `/manifest.webmanifest` 생성 (scope `/`, 모든 카테고리 포함) ② 모든 페이지 head에 `<link rel="manifest" href="/manifest.webmanifest">` 일괄 삽입 ③ `tools/manifest.webmanifest` 정리 ④ SW(`sw.js`) 루트로 이동 + scope `/` 재등록 ⑤ 캐시 자산 목록에 모든 카테고리 + `image/vendor/*` 포함
3. **image/ 도구 manual 실기기 검증** — iOS Safari 카메라(qr-scan), 큰 PDF 메모리(pdf-merge·pdf-split), OCR 한국어 모델 첫 다운로드 흐름
