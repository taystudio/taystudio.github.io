...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-05 (**AdSense 비활성화 — 승인 전 빈 placeholder 숨김 + 외부 스크립트 주석 처리**. `.ad-slot { display:none !important; }` CSS 한 줄로 60개 placeholder hide(위치 보존). 39개 페이지의 `<script async src="...adsbygoogle.js">` 주석 처리 → 외부 fetch 절감. 승인 후 양쪽 다 풀기 = TODO §9.4. + hero hub-title 그라데이션 색 컨벤션 통합 + SEO 최적화 라운드 — Tier 1+2 일괄 — ItemList JSON-LD(tools 36 + text 3, image·video는 이미 적용), 4 카테고리 hub-intro 박스 + hub-faq 섹션 + FAQPage JSON-LD 동기화, HowTo JSON-LD 4 도구(qr-gen·pdf-merge·img-to-pdf·video/compress) → 리치 스니펫 후보, salary description 보강(68→110자), favicon.ico **285KB → 5.2KB**(54배·to-ico packing), 공용 `.hub-intro`·`.hub-faq` CSS, SW v1→v2. + 모바일 video 비-Chrome 환경 자동 안내 배너(삼성 인터넷·Whale 추가 감지) + 첫 사용 hint에 "데스크톱 Chrome 안정적" + ffmpeg 진단 정보 alert 자동 첨부(JS heap·UA·로그 마지막 8줄). 카테고리 = `tools / text / image / video` **4개**. sitemap **60 URL** · 도구 수 **53선**. 다음 세션이 이 문서 보고 이어서 작업.
