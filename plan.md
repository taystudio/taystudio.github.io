...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-06 (**privacy·terms root 이전 — SEO 위치 정합성**. `/tools/privacy/`·`/tools/terms/` → `/privacy/`·`/terms/`. 이유 = `/tools/`는 계산기 카테고리인데 정책은 사이트 전체용 → 의미 부조화 + AdSense·표준 관행상 root 직속이 정답. 옛 위치는 redirect stub(meta refresh + canonical + noindex + JS replace), site-chrome.js footer 2 link + banner "자세히" link 갱신, build-sitemap.sh에 `ROOT_PAGES=("privacy" "terms")` walk 추가(재실행 보존), sitemap 신규 URL 등재·옛 URL 자동 제외(noindex). 도구 수·카테고리 변동 0. **앞선 작업 — 이미지 카테고리 Tier 1 신규 3종(heic-to-jpg·crop·id-photo)** 후 도구 수 55 → 58선, sitemap 63 → 66 URL, 카테고리 5개 유지(계산기 36 + 텍스트 3 + 이미지 9 + PDF 5 + 동영상 5). 다음 세션이 이 문서 보고 이어서 작업.
...
**현 시점(2026-05-06) 사이트 도구 수 = 58선** (계산기 36 + 텍스트 3 + 이미지 9 + PDF 5 + 동영상 5), sitemap **66 URL**. **계산기는 `tools/` 단일 카테고리**로 통합 (건강·임신·육아·자동차 등 모든 계산기는 `/tools/` 아래). **PDF는 별도 카테고리** (image/와 분리, SERP 키워드 풀 강화). **이미지 9선** = 압축·리사이즈·**HEIC→JPG**·**자르기(크롭)**·**증명사진**·QR 생성/인식·OCR·배경 제거(굵게 = 2026-05-06 신규).
