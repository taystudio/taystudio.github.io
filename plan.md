...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-05 (**PWA 후속 — install 버튼 + 아이콘 자산 분리 + TayTools 직관적 이름** — install 버튼(`common/site-chrome.js` site-header 우측, beforeinstallprompt + iOS Safari 모달 fallback) + favicon vs pwa-icon 자산 분리(향후 native와 시각 구분) + PWA 전용 SVG/PNG 디자인(보라 #7c3aed + 큰 WEB 라벨) + short_name "TAYSTUDIO" → "TayTools"(홈 화면 직관적). librsvg 설치로 SVG→PNG 자동화. 사이트 색 컨벤션 정착 = favicon blue / PWA purple+WEB / 향후 native green. + PWA 통합 A안 완료(manifest + root SW) + favicon.ico 추가(네이버 수집 제한 해제) + rotate 미리보기 UX + video wrapper hang fix 부분 검증. 카테고리 = `tools / text / image / video` **4개**. sitemap **60 URL** · 도구 수 **53선**. 다음 세션이 이 문서 보고 이어서 작업.
