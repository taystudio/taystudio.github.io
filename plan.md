...
2. **SEO Tier 1.2 — 카테고리 hub 콘텐츠 깊이** (~2h, **새로 추가**) — `image/index.html`·`text/index.html`·`tools/index.html`이 카드 그리드만이라 thin content. 카테고리 키워드 SERP 약함. 작업: long-form 소개(200~500자) + FAQ 4~5개 + `.updated` 최종 갱신일 + `.sources` 외부 권위 출처. `history/seo/strategy.md` 참조
