...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-07 (**IndexNow 인프라 도입 — Bing·Yandex·Naver(2024 합류)·Seznam에 67 URL 일괄 push** + **Bing Webmaster Tools 가입 진행 중**(msvalidate.01 meta 태그 적용 완료, 검증 대기). Search Console 일일 한도 우회 + 한국 시장 색인 가속. 정적 사이트 친화 — key 파일 1개 + bash 스크립트 1개로 백엔드 0. 첫 실 전송 = HTTP 202 Accepted (67 URLs queued). 도구 수·카테고리 변동 0, sitemap 67 URL 그대로. **앞선 작업 — 저장공간 계산기 출시(2026-05-06, 디지털·기기 카테고리 신설)** + AI 이미지 업스케일링 봉인 + privacy·terms root 이전 + 이미지 Tier 1 신규 3종 + PDF 카테고리 신설. 도구 수 59선(계산기 37 + 텍스트 3 + 이미지 9 + PDF 5 + 동영상 5), tools 카테고리 8개(디지털·기기 포함), sitemap 67 URL. 다음 세션이 이 문서 보고 이어서 작업.
...
> **2026-05-07 IndexNow 인프라 도입 완료 + Bing Webmaster Tools 가입 진행 중**: §9.1 결정 기록 참고. Bing·Yandex·Naver·Seznam 4 엔진에 67 URL 일괄 push 성공(HTTP 202). 향후 새 도구 출시 시 `bash scripts/build-sitemap.sh`만 실행하면 sitemap 갱신 + ping 자동. **Bing Webmaster Tools = msvalidate.01 meta 태그 적용 완료** (검증 대기 — commit/push 후 콘솔 Verify 클릭). 검증 통과 시 IndexNow ping 67건 결과·인덱스 수·검색 키워드 노출 데이터 가시화.
...
- **🔔 쿠팡 파트너스 — 신청 + 친화 도구 후속** (저장공간 계산기 1순위 출시 완료 2026-05-06, §9.1 기록) — **남은 작업** = (1) **쿠팡 파트너스 가입 신청** (즉시 가능, 결과 1주~) (2) **모니터 해상도·인치 변환 + 모니터 추천** 2순위 도구 출시 (2~3h, 디지털·기기 카테고리 2번째 도구. 단가 30~150만 ★★★) (3) 쿠팡 승인 후 **저장공간·모니터 양쪽에 추천 카드 영역 추가** (현재 텍스트 권장만, 카드 자리 placeholder 미존재 → 신규 추가 작업). 차순위 = baby/(분유) · pet/(사료) 재구매 단가. 상세 = `history/candidate/coupang-affiliate-tools.md`
- **🔔 AdSense 승인 후 재활성화** (2026-05-05 비활성 처리 완료) — 승인 메일 받으면 즉시 진행:
  1. **CSS hide 풀기** = `common/css/style.css` 끝의 `/* AdSense 미승인 ... */ .ad-slot { display: none !important; }` 블록 삭제 (3줄). 60개 placeholder 다시 노출
...
10. **영어 시장 진입 — `/en/` path** (~매우 큼, **한국 시장 매출 검증 후 진입**) — 글로벌 트래픽·AdSense 영어 RPM($3-8) 노림. **단순 HTML fork만으론 트래픽 X** — 별도 비즈니스 트랙으로 인식해야 함:
   ...
   - **구조 = B안 (`/en/` path)**: 같은 repo 안에 `taystudios.com/en/`. canonical + `<link rel="alternate" hreflang="en">` 메타로 한↔영 페이지 연결. 정적 우선 전략 일관 + 코드(CSS·라이브러리) 공유
...
> **현 상태 = 검색엔진 3채널(Google SC + Naver SA + Bing WMT) 검증 완료 + IndexNow 67 URL HTTP 200 push 완료, AdSense 추가만 사용자 액션 남음 (2026-05-09 15:50).** Cloudflare Registrar `taystudios.com` 등록 + DNS·GitHub Pages 연결 + Enforce HTTPS ON + 코드 일괄 sed + sitemap rebuild + SW v13 + history 시점 분기까지 완료(2026-05-09 14:00). **GSC** = URL prefix property 검증 + Change of Address 신청 = "이전 중". **Naver SA** = 신규 사이트 검증 + 사용자 인증·sitemap·웹페이지 수집 진행 중. **Bing WMT** = Import from GSC로 자동 verified + sitemap 제출 + IndexNow ping 67 URL HTTP 200 push 완료 (Site Moves UI 제거 발견 → 자동 감지 위임). 진행 갱신 = §9.5.8, 6개월 후 정리 절차 = §9.5.9, 채널별 상세 = `history/migration/google-search-console/index.html` · `history/migration/naver-search-advisor/index.html` · `history/migration/bing-webmaster-tools/index.html`.
...
| 5 | **AdSense 신규 도메인 추가** | ⏳ 사용자 액션 남음 | — | — | `pub-3553250610781349`. ads.txt 신규 도메인 200 OK 확인. **AdSense 미승인 상태** — 도구 페이지에 script 주석 처리된 채 대기 중. 사이트 추가 + 검토 신청 → 승인 후 별도 commit으로 주석 해제 |
