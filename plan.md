...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-05 (**video wrapper hang fix 부분 검증 + rotate 미리보기 UX 개선** — wrapper hang fix는 사용자 콘솔에서 `[ffmpeg]` 로그 정상 출력 확인 = worker spawn·load 단계는 통과. 끝까지 처리·다운로드 완료 검증은 미완. 별도로 rotate 도구에 source 미리보기 박스 + 회전 옵션 5종 즉시 CSS transform 미리보기 + "원래대로" 6번째 옵션 추가 — ffmpeg 처리 전 시각 시뮬로 잘못된 옵션 선택 시간 낭비 방지. 카테고리 = `tools / text / image / video` **4개**. sitemap **60 URL** · 도구 수 **53선**. 다음 세션이 이 문서 보고 이어서 작업.
