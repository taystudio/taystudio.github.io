...
> **작성**: 2026-05-04 / **최종 갱신**: 2026-05-05 (**rotate·to-gif 모바일 메모리 fix — 다운스케일 옵션** — 사용자 정정 "in-app 외 일반 모바일 Chrome에서도 rotate·to-gif fail, trim 빠른모드만 성공" → 진단 = 재인코딩(libx264) 자체가 모바일 single-thread WASM 메모리 한계. fix = rotate에 출력 해상도 select(원본/1080p/**720p default**/480p) + buildArgs scale filter chain(회전 전 다운스케일 = 메모리 절감) + to-gif 모바일 감지 시 width default 480→**320 자동**. 4K·1080p 영상도 720p로 떨어뜨려 인코딩 가능. + 휴대폰 video 에러 진단(`formatVideoError`/`readVideoFile`/in-app 자동 배너) + PWA UX 마감 + install 버튼 + 아이콘 자산 분리 + TayTools + PWA 통합 A안 + favicon.ico + rotate 미리보기 + video wrapper hang fix 부분 검증. 카테고리 = `tools / text / image / video` **4개**. sitemap **60 URL** · 도구 수 **53선**. 다음 세션이 이 문서 보고 이어서 작업.
