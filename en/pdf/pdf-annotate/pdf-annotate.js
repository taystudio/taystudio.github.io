// PDF Edit & Annotate — pdf.js (preview) + pdf-lib (bake). All in the browser.
// Overlay text (KR/EN fonts), pen, highlighter, shapes, white-out onto the PDF.
...
// English page: English fonts on top, Korean fonts below. Default = Helvetica.
const CFG = {
  groupOrder: ['영문', '한글'],
  groupLabel: { '영문': 'English fonts', '한글': 'Korean fonts' },
  defaultFont: 'helvetica',
};

const FB = 'https://cdn.jsdelivr.net/gh/google/fonts@main/ofl';
const FONTS = {
  nanumgothic:   { group: '한글', label: 'Nanum Gothic (Sans)',      ttf: FB + '/nanumgothic/NanumGothic-Regular.ttf' },
  gowundodum:    { group: '한글', label: 'Gowun Dodum (Dotum)',      ttf: FB + '/gowundodum/GowunDodum-Regular.ttf' },
  nanummyeongjo: { group: '한글', label: 'Nanum Myeongjo (Serif)',   ttf: FB + '/nanummyeongjo/NanumMyeongjo-Regular.ttf' },
  nanumpen:      { group: '한글', label: 'Nanum Pen (Handwriting)',  ttf: FB + '/nanumpenscript/NanumPenScript-Regular.ttf' },
  blackhan:      { group: '한글', label: 'Black Han Sans (Heavy)',   ttf: FB + '/blackhansans/BlackHanSans-Regular.ttf' },
  dohyeon:       { group: '한글', label: 'Do Hyeon (Round)',         ttf: FB + '/dohyeon/DoHyeon-Regular.ttf' },
  helvetica:  { group: '영문', label: 'Helvetica (Sans)',           latin: true, std: 'Helvetica',  css: 'Helvetica, Arial, sans-serif' },
  lato:       { group: '영문', label: 'Lato (Modern Sans)',         latin: true, ttf: FB + '/lato/Lato-Regular.ttf' },
  times:      { group: '영문', label: 'Times New Roman (Serif)',    latin: true, std: 'TimesRoman', css: "'Times New Roman', Times, serif" },
  crimson:    { group: '영문', label: 'Crimson (Serif)',            latin: true, ttf: FB + '/crimsontext/CrimsonText-Regular.ttf' },
  courier:    { group: '영문', label: 'Courier (Mono)',             latin: true, std: 'Courier',    css: "'Courier New', monospace" },
  bebas:      { group: '영문', label: 'Bebas Neue (Display Caps)',  latin: true, ttf: FB + '/bebasneue/BebasNeue-Regular.ttf' },
  greatvibes: { group: '영문', label: 'Great Vibes (Signature)',    latin: true, ttf: FB + '/greatvibes/GreatVibes-Regular.ttf' },
  pacifico:   { group: '영문', label: 'Pacifico (Casual)',          latin: true, ttf: FB + '/pacifico/Pacifico-Regular.ttf' },
};
...
let activeText = null;
let annos = [];
let selection = [], selOutline = null;   // selected annotations + selection outline
...
$('pdfInput').onchange = async (e) => {
  ...
  catch (err) { alert('Could not open the PDF. It may be corrupted or password-protected.'); }
};
...
function drawSelection(c, W, H) {
  ...
  selection.forEach(a => {
    if (a.page !== curPage || a.type === 'text') return; // text highlighted via DOM outline
    let bx, by, bw, bh;
...
function textRectNorm(a) { // normalized rect of the rendered text box (not just the anchor point)
...
/* ── eraser ── pen/highlighter: rub out only the part you pass over (split into pieces); text/shape/white-out: whole ── */
...
/* ── Delete / Esc to remove selection ── */
...
$('clearBtn').onclick = () => { if (!annos.some(a => a.page === curPage)) return; if (!confirm('Clear all annotations on this page?')) return; annos = annos.filter(a => a.page !== curPage); activeText = null; redrawOverlay(); renderTextBoxes(); };
$('resetBtn').onclick = () => {
  ...
  $('pdfInfo').textContent = 'Select a PDF'; $('downloadBtn').disabled = true; $('status').textContent = '';
};

async function loadFontBytes(key) {
  if (!FONTS[key]) key = 'nanumgothic';
  if (!fontBytesCache[key]) {
    $('status').textContent = `Loading ${FONTS[key].label} font…`;
...
async function bakeToBytes() {
  ...
  for (const a of annos) {
    ...
    if (a.type === 'white') {
      ...
      if (FONTS[key] && FONTS[key].latin && /[^\x00-\xff]/.test(a.text)) key = 'nanumgothic'; // English font + non-Latin → Korean font fallback
...
$('downloadBtn').onclick = async () => {
  ...
  try {
    ...
    if (err.message === 'ENCRYPTED') alert('Please remove the password from the encrypted PDF first. (/en/pdf/pdf-unlock/)');
    else alert('Error while applying edits: ' + (err.message || err));
    $('status').textContent = '';
